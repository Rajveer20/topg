...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('small')
		.setDescription('Sets a reminder for respawning of Small Aquamarine Island')
	...
	async execute(interaction) {
		...
		const updatedTime = new Date(currentTime + (36 * 60 * 60 * 1000) + (h * 60 * 60 * 1000) + (m * 60 * 1000));
		...
		if(interaction.options.getBoolean('reminder')){
			...
			setTimeout(function(){interactionUser.send('The island ' + interaction.options.getString('coordinates') + ' has respawned!');}, (36 * 60 * 60 * 1000) + (h * 60 * 60 * 1000) + (m * 60 * 1000));
